    ...
    public function changePass(){
        ...
        if ($old!=$old_pass){
            $date = date("Y-m-d");
                    ...
                    $data = array(
                        'due_num' => $due_num,
                        'message' => "Entered password does not matched to your Old Password.",
                        'follow_num' => $follow_num,
                        'acted_num' => $acted_num
                    );
                    if ($this->session->userdata('user_type') == 1) {
    ...
    public function complianceForm() {
        if ($this->session->userdata('logged_in') == null) {
            ...
            if ($this->session->userdata('user_type') == 3) {
                ...
                $data2 = $this->compliance->getCompliance();
                if ($this->session->userdata("user_type")==1) {
                    $this->load->view('adminView/addComplianceForm', array('acted_num' => $acted_num, 'follow_num' => $follow_num, 'due_num' => $due_num, 'doc_id' => $doc_id, 'data2' => $data2));
                }else{
